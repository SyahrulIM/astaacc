        ...
        switch ($marketplace) {
            case 'shopee':
                ...
                if ($type_excel === 'income') {
                    ...
                    if ($processedOrders > 0) {
                        $this->session->set_flashdata('success', "Data Shopee Income berhasil diimport ($processedOrders orders).");
                    } else {
                        $this->session->set_flashdata('error', 'Tidak ada data order yang ditemukan dalam sheet income.');
                    }
                } else {
                    // PLL for Shopee Order
                    ...
                    if ($processedOrders > 0) {
                        $this->session->set_flashdata('success', "Data Shopee Order berhasil diimport ($processedOrders orders).");
                    } else {
                        $this->session->set_flashdata('error', 'Tidak ada data order yang ditemukan.');
                    }
                }
                break;
            ...
            case 'tiktok':
                ...
                if ($type_excel === 'income') {
                    ...
                    if ($processedOrders > 0) {
                        $this->session->set_flashdata('success', "Data TikTok Income berhasil diimpor ($processedOrders orders).");
                    } else {
                        $this->session->set_flashdata('error', 'Tidak ada data order yang ditemukan.');
                    }
                } else {
                    // PLL for TikTok Order
                    $sheet = $spreadsheet->getActiveSheet();
                    $rows = $sheet->toArray(null, true, true, true);
                    $processedOrders = 0;
                    ...
                    if ($processedOrders > 0) {
                        $this->session->set_flashdata('success', "Data TikTok Order berhasil diimpor ($processedOrders orders).");
                    } else {
                        $this->session->set_flashdata('error', 'Tidak ada data order yang ditemukan.');
                    }
                }
            ...
            case 'accurate':
                ...
                if ($processedOrders > 0) {
                    $this->session->set_flashdata('success', "Data Accurate berhasil diimport ($processedOrders orders).");
                } else {
                    $this->session->set_flashdata('error', 'Tidak ada data order yang ditemukan.');
                }
                break;
