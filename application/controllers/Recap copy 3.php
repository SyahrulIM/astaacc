...
class Recap extends CI_Controller
{
    ...
    public function index()
    {
        $acc_recap = $this->db->query("
            ...
            SELECT 
                user.full_name AS full_name,
                DATE_FORMAT(acc_blibli.created_date, '%Y-%m-%d %H:%i:%s') AS created_date,
                acc_blibli.idacc_blibli AS id_data,
                acc_blibli.excel_type AS type,
                CASE 
                    WHEN acc_blibli.is_kotime = 1 THEN 'blibli_kotime'
                    ELSE 'blibli'
                END AS source
            FROM acc_blibli
            JOIN user ON user.iduser = acc_blibli.iduser
            WHERE acc_blibli.created_date IS NOT NULL
    ...
    public function all()
    {
        ...
        SELECT 
            ...
            DATE_FORMAT(d.order_date, '%Y-%m-%d') AS order_date,
            d.refund,
            d.idacc_blibli_detail AS id_detail,
            CASE 
                WHEN b.is_kotime = 1 THEN 'blibli_kotime'
                ELSE 'blibli'
            END AS source
        FROM acc_blibli_detail d
        JOIN (
            SELECT no_faktur, MAX(idacc_blibli_detail) AS max_id
            FROM acc_blibli_detail
            GROUP BY no_faktur
        ) x ON d.no_faktur = x.no_faktur AND d.idacc_blibli_detail = x.max_id
        JOIN acc_blibli b ON b.idacc_blibli = d.idacc_blibli
        ...
        try {
            ...
            switch ($base_marketplace) {
                ...
                case 'blibli':
                    ...
                    foreach ($rows as $i => $row) {
                        if ($i < 3) continue; // Skip header (row 1-2 adalah header)
                        ...
                        $pricePerUnit = floatval($row['I'] ?? 0); // Kolom I = Harga satuan
                        $total = floatval($row['J'] ?? 0); // Kolom J = Total
                        $sellerPromo = floatval($row['K'] ?? 0); // Kolom K = Promo seller
                        $totalPayment = floatval($row['U'] ?? 0); // Kolom U = Total pembayaran

                        // Validasi: skip jika tidak ada order number
                        if (empty($orderNumber)) continue;

                        // Gunakan kombinasi Order ID + Order Item ID sebagai key unik
                        $itemKey = $orderNumber . '|' . $orderItemId;
                    ...
                    $orderSummary = [];
                    foreach ($orderData as $itemKey => $item) {
                        $noFaktur = $item['no_faktur'];

                        if (!isset($orderSummary[$noFaktur])) {
                            $orderSummary[$noFaktur] = [
                                'no_faktur' => $noFaktur,
                    ...
                    foreach ($orderSummary as $noFaktur => $summary) {
                        ...
                        $discount = $summary['total_seller_promo']; // Seller promo sebagai discount
                        ...
                        $detail = [
                            ...
                            'discount' => $discount,
                            'payment' => $payment,
                            'refund' => 0, // Default 0 seperti yang diminta
                            'updated_date' => date('Y-m-d H:i:s'),
                            'updated_by' => $this->session->userdata('username'),
                            'status' => 1
                        ];
                        ...
                        if (!$existingData) {
                            ...
                            $this->db->insert('acc_blibli_detail', $detail);
                        } else {
                            // Update jika sudah ada
                            $this->db->where('no_faktur', $noFaktur)
                                ->update('acc_blibli_detail', $detail);
                        }
                        ...
                        foreach ($summary['items'] as $item) {
                            ...
                            // Cek apakah data sudah ada berdasarkan no_faktur dan sku
                            $existsDetail = $this->db->get_where('acc_blibli_detail_details', [
                                'no_faktur' => $noFaktur,
                                'sku' => $item['sku']
                            ])->row();
                            ...
                            if ($existsDetail) {
                                ...
                                $detail_order['created_date'] = date('Y-m-d H:i:s');
                                $detail_order['created_by'] = $this->session->userdata('username');
                                $this->db->insert('acc_blibli_detail_details', $detail_order);
                                $processedItems++;
                            }
                        }
                    }
                    ...
                    $this->session->set_flashdata(
                        $processedOrders > 0 ? 'success' : 'error',
                        $processedOrders > 0 ? "Data Blibli " . ($is_kotime ? "Kotime" : "Asta") . " berhasil diimport ($processedOrders orders, $processedItems items)." : 'Tidak ada data order yang ditemukan.'
                    );
                    break;
    ...
    public function detail_faktur()
    {
        ...
        if ($marketplace === 'tiktok') {
            ...
            $acc_detail_details = $this->db->get('acc_blibli_detail_details')->result();
    ...
    public function detail_payment()
    {
        ...
        if ($marketplace === 'accurate') {
            ...
            $this->db->where('idacc_blibli', $idacc_recap);
            $acc_detail = $this->db->get('acc_blibli_detail')->result();
        } else {
